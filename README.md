...
> Redis에 캐시/증분 연산을 위임함으로써 DB는 적은 I/O만 처리
> 또한 관계형 데이터베이스에 직접 쓰게 되면, Lock 경합, I/O 병목이 발생할 수 있기 때문에
> Redis 를 통해 조회 수 처리를 진행했습니다.
> 데이터 정합성을 위해 일정 조회수가 넘으면 주기적 백업까지 구현했습니다.

- 페이징 및 정렬 기능
> 페이징 같은 경우 대량의 데이터를 효율적으로 처리하기 위해 최적화를 진행했습니다.
> 인덱스 같은 경우 조회 성능을 높이기 위해 사용했습니다.
> 또한 count query 같은 경우 1 ~ 10 페이지가 만약 10개라고 한다면 101개의 쿼리만 조회하면 됩니다.
